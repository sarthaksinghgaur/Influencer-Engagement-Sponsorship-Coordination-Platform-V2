          ...
          <div class="card-body">
            ...
            <div class="statistics mt-4">
              ...
              <ul>
                ...
                <li><strong>Total Ad Requests:</strong> {{ stats.ad_requests }}</li>
                ...
                <li><strong>Flagged Campaigns:</strong> {{ stats.flagged_campaigns }}</li>
                <li><strong>Flagged Ad Requests:</strong> {{ stats.flagged_ad_requests }}</li>
              </ul>
            </div>
            ...
            <div class="button-container mt-4">
              <button @click="navigateTo('AdminViewUsers')" class="btn btn-primary">Monitor Accounts</button>
              <button @click="navigateTo('AdminViewInfluencers')" class="btn btn-primary">Monitor Influencers Database</button>
              <button @click="navigateTo('AdminViewSponsors')" class="btn btn-primary">Monitor Sponsors Database</button>
              <button @click="navigateTo('AdminViewCampaigns')" class="btn btn-primary">Monitor Campaigns Database</button>
              <button @click="navigateTo('AdminViewAdrequests')" class="btn btn-primary">Monitor Ad Requests Database</button>
...
export default {
  ...
  methods: {
    fetchAdminDashboard() {
      ...
      const cacheExpiry = localStorage.getItem('adminDashboardStatsExpiry');

      if (cachedStats && cacheExpiry && new Date() < new Date(cacheExpiry)) {
    ...
    fetchPendingSponsors() {
      ...
      const cachedPendingSponsors = localStorage.getItem('pendingSponsors');
      const cacheExpiry = localStorage.getItem('pendingSponsorsExpiry');

      if (cachedPendingSponsors && cacheExpiry && new Date() < new Date(cacheExpiry)) {
        this.pendingSponsors = JSON.parse(cachedPendingSponsors);
      } else {
        axios
          ...
          .then((response) => {
            ...
            localStorage.setItem('pendingSponsors', JSON.stringify(this.pendingSponsors));
            localStorage.setItem('pendingSponsorsExpiry', new Date(Date.now() + 1 * 60 * 1000));
          })
          .catch((error) => {
