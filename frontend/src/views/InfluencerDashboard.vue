...
export default {
  ...
  methods: {
    fetchInfluencerData() {
      ...
      const cachedInfluencerData = localStorage.getItem('influencerDashboardData');
      const cacheExpiry = localStorage.getItem('influencerDashboardDataExpiry');

      if (cachedInfluencerData && cacheExpiry && new Date() < new Date(cacheExpiry)) {
        const cachedData = JSON.parse(cachedInfluencerData);
        this.influencer = cachedData.influencer;
